...
export default function Profile() {
  ...
  const save = async (e) => {
    ...
    try {
      const patch = {}
      if (name.trim()) {
        if (profile?.name !== undefined || profile?.display_name === undefined) patch.name = name.trim()
        if (profile?.display_name !== undefined) patch.display_name = name.trim()
        if (Object.keys(patch).length === 0) patch.name = name.trim()
      }
      ...
      setMsg({ ok: false, text: err.message || 'Could not update profile. Your permissions may not allow it.' })
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <div>
      ...
      <Card title="Work information" sub="Assigned by your supervisor — read only">
        <dl className="kv">
      ...
      {editing && (
        <Card title="Edit basic info" sub="Only name and phone can be changed here. Role, department and section are managed by your supervisor.">
